...
DONE — acceptance criteria covered by the deterministic suite and checked in real Claude Code with the real Codex reviewer (approval and findings). `$ccc-review current` was checked in the real Codex TUI too, including the fix it led to (instructions from the hook).
...
Real Codex TUI (2026-09-20, codex-cli 0.155.1): `$ccc-review current přidal jsem divide(a, b), má dělit` was not blocked, the Claude reviewer audited the planted bug (`CCC-001 [high] math.js:3`, `CCC-002` missing test), Codex presented both findings with its own assessment and changed nothing (`git diff --stat`: only the user's two lines). But Codex said "the ccc-review skill is not available" and wrote a short review of its own instead of Task/Plan/Report: the session record shows that Codex neither lists the skill for the model (`allow_implicit_invocation: false`) nor injected its text for the typed mention. Fixed: the Codex hook now returns the instructions itself as `hookSpecificOutput.additionalContext` (supported for `UserPromptSubmit`, checked in `codex-rs/hooks/src/events/user_prompt_submit.rs`), from `auditInstructions()` in `common.ts`, which also says that the message is a report, not the writer's own review. `codex-host.test.ts` "`current` reaches the model…" asserts the exact output keys and the content. Claude Code keeps using the skill text, which worked in real use. Re-checked in the real Codex TUI after upgrading the installed plugin: Codex wrote a structured Task/Plan/Report (no "skill is not available"), the Claude reviewer returned `CCC-001 [high] math.js:3` and `CCC-002 [medium]` and noted that the writer's report was truthful, and Codex presented both with its own assessment and changed nothing (`git diff --stat` unchanged).

Real project (2026-09-21, 1.0.0): a second `/ccc-review:current` in one session was approved, but the user saw no answer and Claude later wrote "the answer has not reached me yet". The session transcript showed the hook had returned `systemMessage: "… Codex APPROVED (round 1/1) …"`: Claude Code records that for the user only, the writer never sees it, and it is easy to miss under a long report. Fixed in 1.0.1: every audit outcome (not only findings) is returned as `decision: "block"` with `auditResult()`, so the writer presents the verdict; the task is already inactive, so that completion is not reviewed. `on` is unchanged. Scenarios "is not blocked, reviews once and approval ends it" and "a failing reviewer is not approval" assert it for both directions.
