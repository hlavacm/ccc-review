...
### Commands renamed before release (2026-09-19)

The single skill `ccc-review` made the command `/ccc-review:ccc-review on|off|status`. It is now one skill per action: `/ccc-review:on [task]`, `/ccc-review:off`, `/ccc-review:status` (`skills/{on,off,status}`, matcher `^ccc-review:(on|off|status)$`). Earlier notes in these feature files use the old name. Measured first with a throwaway plugin in Claude Code 2.1.278, because the docs do not say: `command_name` arrives namespaced (`probe:on`) with `command_source: "plugin"`; the built-in `/status` does not fire `UserPromptExpansion` at all; a bare `/on` does not resolve to a plugin skill. So the hook accepts exactly the three namespaced names and no longer the bare `ccc-review`, and a skill called `status` cannot shadow the built-in. Tests: `claude-workflow.test.ts` "command matcher selects the three commands and nothing else" and "every command the matcher selects has a skill, and the reverse", `claude-host.test.ts` "other commands are ignored, including bare and look-alike names" and "off and status ignore trailing arguments" (55 tests failed against the old hook first). Codex is unchanged (`$ccc-review on|off|status`).
