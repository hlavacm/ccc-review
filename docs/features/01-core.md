...
- `src/core/state.ts` — `TaskState` (version 1), `createTaskState()`, `saveState(dir, state)` (atomic tmp + rename to `<dir>/<taskId>.json`), `loadState(dir, taskId)` (`undefined` when missing, `StateError` when corrupt). The state directory is chosen by the host (planned for 02: `<git-common-dir>/ccc-review/`).
