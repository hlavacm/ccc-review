...
- `src/hosts/claude-code/`: the plugin's hook code. `cli.ts <command|prompt-submit|stop>` → `runHook` in `hooks.ts`, which never throws: errors become `systemMessage` (or a blocked command), never block/approve. `handleStop` claims each completion (`sha256(last_assistant_message)`, exclusive `wx` create) so a duplicate/concurrent Stop never starts a second round. The plugin itself is the repo root: `.claude-plugin/`, `hooks/hooks.json`, `skills/{on,current,off,status}/SKILL.md`; the commands are `/ccc-review:on [task]`, `/ccc-review:current [note]`, `/ccc-review:off`, `/ccc-review:status` (one skill per action, because Claude Code names a plugin skill `<plugin>:<skill>` and reports it that way in `command_name`; a bare `status` would be the built-in). Hook matchers made only of letters/digits/`_-,| ` are exact-string matches in Claude Code; with `:` the matcher is an unanchored regex, hence `^ccc-review:(on|current|off|status)$`.
- `src/hosts/common.ts`: the host-side workflow both hosts share (activation, sessions, claims, `reviewCompletion`, history, writer/user texts derived from the task's writer/reviewer). A host only drives tasks whose `writer` is its own agent. Unreadable state never wedges a session: `on` replaces it, `off` resets the session. A completion is skipped (no claim, no round, `systemMessage` only) when nothing can have changed: clean baseline, same HEAD, clean tree now (`unchangedSinceCleanActivation`); test harnesses therefore touch a file before `stop()` unless `simulateWork` is off. `runCommand` returns the text to show (the host blocks the prompt) or `undefined` to let the prompt through: an armed `current` (one-round audit of the uncommitted changes, `Session.audit`, `maxRounds: 1`) needs the writer to write task, plan and report: Claude Code gets the instructions from `skills/current`, Codex from the hook as `hookSpecificOutput.additionalContext` (`auditInstructions`), because Codex does not load the skill for a typed mention; every result then goes back to the writer as a block to be shown, not fixed (`auditFeedback` for findings, `auditResult` for approval/needs human/failure), because a `systemMessage` never reaches the writer.
