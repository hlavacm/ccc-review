...
- `src/hosts/common.ts`: the host-side workflow both hosts share (activation, sessions, claims, `reviewCompletion`, history, writer/user texts derived from the task's writer/reviewer). A host only drives tasks whose `writer` is its own agent.
- `src/hosts/codex/`: the Codex plugin hooks (`.codex-plugin/plugin.json` → `codex/hooks.json`, `codex/skills/cccr`). `UserPromptSubmit` handles `$cccr on|off|status` (blocked, never reaches the model) and records the task; `Stop` claims `sha256(turn_id \0 last_assistant_message)`. Codex rejects unknown output keys and a block without a reason.
...
- `test/helpers/`: `TemporaryGitRepository` (real temp repos), `FakeReviewer` (scripted results or throws, and records requests), `FakeCodex`/`FakeClaude` (real executables wrapping `test/fixtures/fake-*.ts` via `FakeCli`; scripted steps, records argv/stdin/cwd), `CodexHostHarness`, `hostScenarios()` (workflow scenarios run for both directions), `ClaudeHostHarness` (realistic hook payloads), temp-dir utils. `test/integration/claude-workflow.test.ts` runs the exact `hooks.json` commands as subprocesses.
...
- Review activation is explicit (e.g. `/cccr on|off|status`); inactive sessions are unaffected.
